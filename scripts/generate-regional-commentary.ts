 ...
 *
 * 🔴 **이 값을 바꾸면 `realtrade-story` 도 같이 바꿔야 한다.**
 * 그쪽 `src/lib/format/weekly-summary.ts` 의 `EXPECTED_LAG_WEEKS` 가 이 값의 짝이다 —
 * "몇 주까지가 정상이고 그 뒤부터 고장인가"를 그 숫자로 판정해 홈 카드에 경고를 띄운다.
 * **저장소가 달라 아무도 강제하지 않는다.** 여기만 키우면 그쪽이 정상을 고장이라 부르고
 * (매주 영구히 경고가 켜져 아무도 안 읽게 된다 — 2026-09-23에 실제로 그럴 뻔했다),
 * 여기만 줄이면 그쪽이 진짜 고장을 정상이라 부른다.
 *
