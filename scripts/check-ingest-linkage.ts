 ...
 *
 * [부산 16개 구 — 2026-08-19 수집 재개, Phase 41 / ADR-064. 기본값에는 넣지 않는다]
 *
 * 부산은 운영권역과 임계가 다르다(부산 65 vs 운영권역 80). 그래서 기본 범위에 넣는
 * 대신 명시 호출로 잰다:
 *
 *   npx tsx scripts/check-ingest-linkage.ts --since-hours=8 \
 *     --sgg=26110,26140,26170,26200,26230,26260,26290,26320,26350,26380,26410,26440,26470,26500,26530,26710 \
 *     --min-linked-pct=65
 *
 * `molit-backfill-once.yml` 의 `min_linked_pct` 입력이 이 경로다.
 *
 * 부산 전체 실측 연결률(2026-08-20, Phase 41 백필 완료 시점, 41-08 실측): **아파트
 * 67.2%(631,612건) / 연립다세대 2.1%(180,103건)**. 대조군: 운영권역 94.6% / 경남 기타
 * 16곳 64.7%. 아파트 67.2% 는 D-07 임계(65%, 경남 기타 실측 기준) 통과.
 *
 * 🔴 **운영권역 94.6% 는 수동 별칭 보정이 누적된 결과다**(`complex_aliases`, 매칭 로직
 * 개선, 토월성원 병합 등). K-apt 시딩만 받은 지역의 자연 수준은 60%대(경남 기타
 * 64.7%, 부산 아파트 67.2%)다. **부산에 90% 를 요구하면 상시 빨간불이 되고, 그러면
 * 아무도 안 본다** — 위 88.7%/50.5% 대조가 이미 경고한 바로 그 함정이다.
 *
 * 연립다세대 2.1% 는 매칭 고장이 아니라 구조적이다 — `complexes` 는 K-apt 기반
 * **아파트 단지 마스터**라 연립·다세대에 대응 단지가 애초에 없다(41-05-07 §5).
 * 부산을 재려면 아파트/연립을 나눠 읽어야 한다. 합산(52.8%)만 보면 "매칭 악화"로
 * 오판한다.
 *
 * ⚠️ **감시 공백 — 알고 남긴 것**: `molit-daily.yml` 의 연결률 검사 스텝은 인자 없이
 * `check-ingest-linkage.ts --since-hours=6` 을 호출하므로 **기본 범위(창원·김해)만
 * 잰다.** 일배치가 매일 적재하는 부산 신규 거래는 이 감시의 판정 대상이 아니다.
 * 좁히려면 임계가 다른 두 범위(운영권역 80 / 부산 65)를 각각 재는 스텝 분리가
 * 필요한데, 이는 별칭 보정 후속 작업과 함께 판단할 사항이라 이번엔 하지 않았다.
 */
