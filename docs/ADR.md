...
### ADR-065 — 거래↔단지 매칭에 지번 게이트: 이름 단독 매칭을 끝낸다

**결정**: 거래를 단지에 붙일 때 **지번을 근거로 쓴다.** `complex_canonical_jibun` 테이블에
단지별 확정 지번을 거래 다수결로 물질화하고, `match_complex_by_admin` 이 그 지번을 참조해
**유일하게 한 단지를 가리킬 때만** 이름 매칭 결과를 정정한다 (`20260821090000` ~ `110000`).

**왜 — 규약이 지켜지지 않고 있었다**: `CLAUDE.md` 가 *"단지명 단독 매칭 금지 — 항상 좌표+이름
복합 매칭"* 을 CRITICAL 로 못박았는데, **거래 연결 경로는 이름만 보고 있었다.**
`match_complex_by_admin` 이 `p_umd_nm`(동)을 받으면서 3단계에서만 쓰고, 대부분은 1단계
trigram(≥0.9)에서 확정됐다. 전수 조사(운영권역 1,897 + 부산 1,767 단지 / 거래 89만건,
카카오 63회 교차검증) 결과 오연결 **1,659건**이 나왔다 — 시영장미(내서읍 중리)에 7,544m
떨어진 양덕동 거래 67건, 반림럭키에 3,918m 떨어진 가음동 거래 319건 등. 이동 329건 +
끊기 1,330건으로 정리했다.

**왜 "1단계에 동 필터"가 아닌가**: 기존 주석의 *"동 필터 없음 — 기존 매칭 회귀 방지"* 판단이
옳았다. 큰 단지는 여러 법정동·여러 필지에 걸친다 — 대동다숲(원계리/삼계리)은 카카오 실측
**0m 같은 부지**이고, 율하e편한세상은 한 지번 밖에 506건이 있다. 동을 강제하면 이들이 전부
미연결로 떨어진다. 그래서 후보를 좁히는 **필터**가 아니라, 지번이 확정적일 때만 정정하는
**게이트**로 만들었다.

**유일성이 핵심이다**: 첫 구현이 `limit 1` 이라 한 지번을 여러 단지가 공유할 때 한쪽으로
쏠렸다. 실측 14그룹/31곳이고 대부분 **1차·2차**다(외동 705 = 주공1/주공2, 안동 BL-2-2 =
푸르지오 하이엔드 1단지/2차). 1차·2차를 가르려던 장치가 오히려 뭉갤 뻔했다 —
`matching-ambiguous-20260805.md` 가 이미 경고한 실패 모양이다. **지번이 정확히 한 단지를
가리킬 때만 작동하고, 충돌하면 꺼져 이름에 맡긴다.**

> **두 근거는 상호 보완한다.** 이름 같고 지번 다르면 **지번**이 가른다(메트로시티 1차/2차/석전).
> 지번 같고 차수 다르면 **이름**이 가른다(주공1/주공2). 어느 한쪽만으로는 못 푼다.

**부수 결정**:
- `complexes.status` 에 **`out_of_region`** 추가 — 운영권역 밖 단지 9곳(부산·창녕·양산·고성)
  격리용. `demolished` 재사용은 거부했다. 철거되지 않은 건물을 철거됐다고 기록하면 나중에
  잘못된 결론을 부른다. 거래 288건은 실재하므로 지우지 않고 연결만 끊었다
- `complex_integrity_counts` **파일화**(`20260821140000`) — 프로덕션에만 있던 drift.
  정의를 보고서야 `empty_kapt` 59→61 이 오염이 아니라 `household_count > 0` 조건 때문에
  **모수가 넓어진 것**임을 판정할 수 있었다

**교훈 — `db push` 성공은 검증이 아니다**: 유일성 검사를 `min(j.complex_id)` 로 썼는데
PostgreSQL 에 `min(uuid)` 집계가 없어 함수가 **어떤 인자로도 호출 즉시 실패**했다.
plpgsql 은 지연 바인딩이라 본문 오류가 실행 시점에야 드러난다(`20260819030000` 과 같은 부류).
적용 직후 실제로 호출해 검증했기에 즉시 잡았다.

**한계**: 게이트 보호율은 1,417/1,888 = **75.1%** 다. 나머지는 거래가 적어 다수결이 서지
않는 단지들이다. `jibun_address` 대량 백필은 값이 작다 — 없는 751곳 중 636곳(85%)이 이미
확정지번으로 보호되고 있다.
