...
must_haves:
  truths:
    ...
    - "generateMetadata의 og:description이 displayDate 최고가 거래 요약을 동적으로 포함한다 (D-07)"
...
// ChampionComplex 타입 (Plan 02 결과물):
// interface ChampionComplex {
//   complexId: string; complexName: string; urlSlug: string | null;
//   si: string | null; gu: string | null;
//   avgPricePerPyeong: number;      -- 만원/평
//   priceChange30d: number | null;  -- 비율
...
**Metadata (D-07) — 동적 og:description 포함:**
```typescript
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  // 동적 og:description: displayDate 최고가 거래 요약 (D-07)
...
**네이버 카페 URL:** `https://cafe.naver.com/chanwonbds` (실제 카페 URL로 교체 가능하며 일단 이 값 사용)
  </action>
  <verify>
  ...
  <done>
    ...
    generateMetadata의 og:description이 displayDate 최고가 거래를 동적으로 반영한다.
  </done>
</task>
...
| RSC → Supabase | createReadonlyClient() (anon key), 쿼리 결과 공개 데이터 |
...
# 동적 og:description — topTx 쿼리 존재 확인
grep -c "topTx" C:/Users/jung/coding/bds/src/app/rankings/page.tsx

# NEW HIGH 뱃지 색상 확인 (#ea580c)
grep "ea580c" C:/Users/jung/coding/bds/src/app/rankings/page.tsx

# 대장단지 카드 txCount90d 사용 확인
grep "txCount90d" C:/Users/jung/coding/bds/src/app/rankings/page.tsx
...
3. 대장단지 섹션 3개 카드 (창원/마산/김해), txCount90d 표시
...
9. og:description이 displayDate 최고가 거래를 동적으로 반영 (topTx 쿼리)
10. `npm run build && npm run lint` 통과
</success_criteria>

<output>
완료 후 `.planning/phases/27-rankings-page/27-03-SUMMARY.md` 생성.
포함 내용: 생성된 파일 목록, 주요 섹션 구조, OG 메타 형식 (동적 og:description), nav 변경 내역, 빌드 결과.
</output>
