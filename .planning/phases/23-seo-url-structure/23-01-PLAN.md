...
must_haves:
  ...
  truths:
    ...
    - "getSiPageData('창원시') → guList 반환, getSiPageData('김해시') → dongList 반환 (각각 avgPrice 포함)"
    - "getDongPageData → complexes 배열 + avgPrice: number | null"
...
<task id="T-02" type="execute" tdd="true">
  <title>Task 2: seo-hierarchy.ts 데이터 함수 + seo-hierarchy.test.ts (TDD)</title>
  <behavior>
    - getSiPageData('창원시') → guList 반환, guList[0].avgPrice가 number 또는 null (W1)
    - getSiPageData('김해시') → dongList 반환, dongList[0].avgPrice가 number 또는 null (W1)
    - getDongPageData → complexes 배열 + avgPrice: number | null (동 평균 평당가, W1)
    - getComplexBySlug('') → null (Pitfall 6 방어 — DB 쿼리 없이 즉시 반환)
    - getComplexBySlug('창원시/성산구/내동/대우2차') → ComplexDetail & { url_slug: string }
  </behavior>
  <read_first>
  ...
  <action>
    **[RED] 먼저 `src/lib/data/seo-hierarchy.test.ts` 작성 (구현 파일 없으므로 실패해야 정상):**
    ...
    ```
    → `Cannot find module './seo-hierarchy'` 오류. RED 확인.

    ---

    **[GREEN] 다음으로 `src/lib/data/seo-hierarchy.ts` 작성:**
  ...
  <acceptance_criteria>
    - npx vitest run src/lib/data/seo-hierarchy.test.ts → 5개 이상 PASS (avgPrice 테스트 포함)
    ...
    - grep -c "avgPrice" src/lib/data/seo-hierarchy.ts → 3 이상 (DongPageData + getSiPageData + getDongPageData)
  </acceptance_criteria>
  <done>5개 이상 Vitest 테스트 PASS (avgPrice 포함), 4개 데이터 함수 export 확인</done>
</task>
...
<verification>
- npx vitest run src/lib/utils/url-slug.test.ts → 10개 PASS
- npx vitest run src/lib/data/seo-hierarchy.test.ts → 5개 이상 PASS (avgPrice 포함)
...
- DongPageData.avgPrice + SiPageData.dongList[].avgPrice 모두 포함 (W1)
