...
### 🚨 origin/main 56개 커밋 미푸시 발견 (2026-07-06, 백필 이어서 진행 중 발견)

전날까지의 Phase 33 작업(33-00~33-10 전체 + 관련 fix 커밋) 및 그 이전 vision 문서 작업까지 포함해 **로컬 main이 origin/main보다 56개 커밋 앞서 있었고, origin/main의 마지막 커밋은 2026-06-30**이었음이 발견됨. 즉 GitHub Actions 크론(molit-daily, ingest-sgis 분기별, kapt-facility-enrich 등)과 Vercel 프로덕션 배포가 전부 Phase 33 이전 코드로 동작 중이었음 — 방금 트리거한 관리비 백필이 이미 33-06에서 고친 `kapt.ts`의 `server-only` import 버그로 재실패한 것이 발견 계기. `git merge-base`로 확인 결과 origin/main은 로컬 히스토리의 순수 조상(fast-forward 가능, 충돌 없음) — 사용자 확인 후 즉시 push 완료(`b3a2124`→`b0638fe`). 이후 이어지는 모든 신규 커밋도 push까지 완료해야 실제 크론·배포에 반영됨을 유의.

### 백필 이어서 진행 + 2차 하드코딩 스윕 (2026-07-06, 오케스트레이터 직접 실행)

1. **origin/main 푸시** (위 항목) — 이후 크론·배포 정상화
2. **좌표 지오코딩 폴백 추가**: `geocode-complexes.ts`가 카카오 "장소" 키워드 검색만 사용해 `road_address`가 있어도 못 찾는 사례 다수 확인 → 주소 검색 API 폴백 추가, 재실행으로 90개 실패 중 46개 추가 복구. **신규 지역(788개) 좌표는 100% 완료**(project 전체 잔여 실패 44개는 신규 지역과 무관한 기존 소규모 건물)
3. **관리비 시설정보 백필** (`kapt-facility-enrich-once.yml`) — 1차 트리거는 origin 구버전 코드로 실패, push 후 재트리거했으나 처리 속도가 기존 실측(2,031개/9m46s) 대비 20배 이상 느려짐(295개/25분) → 30분 timeout으로 cancelled(344/2,822 처리). 원인 불명(API단 지연 추정) — `timeout-minutes: 30→180`으로 상향 후 재트리거, 진행 중
4. **관리비 엑셀 임포트 하드코딩 발견**: `import-management-cost.ts`가 `sgg.includes('창원')||sgg.includes('김해')`로 하드코딩되어 있어 신규 지역 K-apt 엑셀을 넣어도 전부 스킵될 뻔했음(9번째 하드코딩 사례) → regions 테이블 동적 조회로 수정
5. **2차 전체 스윕** (general-purpose 에이전트 위임 + 오케스트레이터 직접 수정) — 실거래가/시설 파이프라인 외 영역에서 하드코딩 13건 추가 발견·수정 (커밋 `2aa2153`):
   - **실제 운영 크론**: `crawl-presale-news.ts`(매주 월 09:00 KST) — 분양뉴스 검색 쿼리가 창원/김해/마산/진해로 한정
   - **좌표 보정 도구 3종**: `fix-null-coords.ts`/`fix-wrong-coords.ts`/`fix-null-coords-jibun.ts` — BBOX+isInRegion() default:false로 신규 지역 좌표를 영구 거부하던 구조적 버그. 이번 세션에서 필요해진 "잔여 좌표 수동 보정" 작업 자체가 이 버그 때문에 막혀있었음
   - **K-apt 코드 조회**: `kapt-code-lookup.ts` — sgg_code 하드코딩 + PostgREST 1,000행 캡 동시 발견(10번째 캡 사례)
   - **학교알리미 스크래핑 3종**(11월 재실행 예정, CLAUDE.md): `scrape-school-advancement/details/contact.ts` — TARGETS가 창원/김해 GUGUN_CODE 2개뿐. 학교알리미 공개 API 실호출로 `GUGUN_CODE = sgg_code+'00000'` 패턴 검증 후 22개 지역 전체로 확장
   - **통학구역 임포트**: `import-school-districts.ts` — `SGG_CD==='121'`(창원 의창구만) 단일 체크가 **Phase 33과 무관하게도 기존 버그**였음(SHP 원본에 창원 5개 구가 개별 코드로 존재, 나머지 4개 구 학군 데이터가 원래부터 누락 중이었음). `sdCd==='48'`(경남 전체)로 일반화 → dry-run 결과 초등 통학구역 선택 627개로 증가
   - **학원 NEIS 수집**: `fetch-hagwon-neis.ts` — 기본 대상 지역 창원/김해 2개 → regions 동적 조회
   - **지오코딩/실거래연결 보조 스크립트**: `geocode-to-sql.ts`, `link-transactions.ts` — SGG_LABEL 하드코딩 제거. `link-transactions.ts`의 2차 폴백 쿼리가 대상 지역과 무관하게 `"창원"`으로 고정되어 있던 버그도 함께 수정
   - **실거래 매칭 정규화**: `naver-land.ts normalizeComplexName()`, `arch-hub/client.ts matchArchHub()` — 실제 격주/월간 크론이 쓰는 단지명 정규화 함수의 지역명 접두어 제거 목록에 신규 16개 지역명 추가(동·읍면 단위 별칭은 지역별 리서치 필요해 defer)
   - **의도적 defer 유지**: `map-naver-complexes-playwright.ts`(BBOXES, 지리적 튜닝 필요), `reb.ts`(clsId 리서치 필요), `cafe-ingest`(별도 phase), `card-news/scripts/generate.js`(콘텐츠 결정)

**동일 버그 클래스 누적 발생 횟수**: 하드코딩 지역 배열 13건(2차 스윕분) + 기존 발견분 = 총 20건 이상. PostgREST 1,000행 캡은 이번에 2건 추가 발견(kapt-code-lookup.ts 포함 총 3건). 전체 저장소 sweep을 2차례(2026-07-03, 2026-07-06) 진행했음에도 매번 신규 발견이 나온 것으로 보아, 유사 확장(3단계 전국 확장 등) 착수 전 반드시 전체 sweep을 재수행할 것.
