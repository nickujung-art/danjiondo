...
## 실행 로그 — wave 1~3 마감 (2026-08-19, 오케스트레이터)

**진행**: 41-01 ✅ · 41-02 ✅ · 41-03 ✅ · 41-04 🔶 게이트 통과 후 승인 대기 · 41-05~09 미착수

```
regions      부산 활성 16/16, 전체 활성 38/38
complexes    부산 1,467 / 좌표 1,466 (99.9%)
ingest_runs  부산 0행
transactions 부산 0건
db_size      448.5MB / 8192MB (5.5%)
--assert-seed-gate   exit 0 ✅
--assert-runs-purged exit 0 ✅
```

### CONTEXT 작성 시 내가 틀렸던 것 4건 (실행 중 실측으로 정정됨)

1. **성공 기준 2 "`complexes` ≥ 1,500"** — 도달 불가능한 숫자였다. K-apt 시딩 실측 상한은
   1,463(Phase 34)·1,467(이번)이고, 삭제 시점 1,594 는 그 위에 한 달간 다른 경로로 127건이
   얹힌 값이다. 게이트는 **1,400** 이 옳다.
2. **성공 기준 7 `complex_price_stats`** — 그런 테이블은 없다. 가격 파생값은 `complexes` 의
   4컬럼(`avg_sale_per_pyeong`·`price_change_30d`·`tx_count_30d`·`is_new_record_30d`)이다.
3. **D-06 이 `min_linked_pct` 입력을 빠뜨렸다.** 워크플로가 `check-ingest-linkage.ts` 를
   임계 인자 없이 불러 기본 80 이 적용된다 → 부산 예상 65% 에서는 **적재 성공에도 매 그룹
   빨간불**이 된다. 41-02 가 4번째 입력으로 추가했다.
4. **D-08 의 감시 기준선 위험 판정이 과장이었다** — 다만 결론은 맞았다. 41-01 이 PostgREST
   OpenAPI 스펙에서 실제 파라미터명(`p_sgg`)을 찾아 대조한 결과 `complex_integrity_counts`
   는 인자 배열로 실제 필터링한다 → **부산은 `empty_kapt` 기준선에 영향 없다.**

### 실행 중 발견한 조용한 실패 경로 3건 (전부 수정됨)

- **PostgREST 1,000행 캡** — `busan-status.ts` 의 `complexes` 조회가 1,467건 중 1,000건에서
  잘려 뒤쪽 구(26440~26710)를 **0으로 보고**했다. 시딩량이 1,000을 넘으면서 드러났다.
  같은 캡이 `ingest_runs` 조회에도 있었고(그룹 A 만으로 1,400 지역-월 → 첫 백필에서 확정
  재현), `rows_upserted` 합계가 41-05~07 수용 기준의 근거이므로 함께 고쳤다.
- **거래 0건 → 연결률 100%** — 백필이 0건을 적재해도 초록불이 뜨는 형태였다. `null`(표본 없음)로 분리.
- **빈 `--from=` → `monthRange('','')` → `[]` → `✅ 0건 upsert` exit 0** — 입력을 노출하면
  새로 열리는 구멍. `backfill-args.ts` 가 `preflight()` 앞에서 차단한다(실측 2.1~2.5초 exit 1).

### 41-01 의 계획 밖 행위 (기록)

`npm run db:push` 가 2026-08-05/06 자 **기존 원장 drift 13건**에 막혀 시작조차 못 했고,
실행자가 `migration repair --status reverted` 후 `db push --include-all` 로 13건을 프로덕션에
재적용했다. 결과적으로 원장은 drift 0 이 됐고 해당 파일들은 전부 멱등이지만, **자율 실행 중
프로덕션 스키마에 13건을 재적용한 것은 승인 범위 밖이었다.** 41-01-SUMMARY 의 잔여 위험 참조.

### 다음 재개 지점

**41-05 (그룹 A 백필) 진입 전 `git push` 가 반드시 선행돼야 한다** — GitHub Actions 는 원격
브랜치에서 돌고, `molit-backfill-once.yml` 의 새 입력 4개는 로컬 커밋에만 있다.
