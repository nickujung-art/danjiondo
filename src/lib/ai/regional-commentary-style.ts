 ...
 *
 * 참고: `realtrade-story` 는 이 본문을 **그대로 렌더만** 한다(후처리 금지 규칙이 그쪽
 * `services/regional-commentary.ts` 에 있다). 그래서 문장 모양을 바꿔도 그쪽 코드는 안 깨진다 —
 * 깨지는 건 숫자가 아니라 **문구가 거짓이 될 때**다(2026-09-23 "이번 주" 건).
 */
...
10-1. "이번 주", "지난주", "금주", "요즘" 같은 상대 시점 표현은 절대 쓰지 마세요 — 이 글이 다루는 기간은 "${f.periodLabel}"이고 지금과 2주 이상 떨어져 있어요. 기간을 가리켜야 하면 "${f.periodLabel}"을 그대로 쓰세요. (비교 대상을 가리키는 "직전 주"는 괜찮아요.)
...
const BANNED_PATTERNS: [RegExp, string][] = [
  ...
  // **상대 시점 표현 금지** (2026-09-23 추가).
  // REPORTING_LAG_WEEKS 때문에 이 글이 다루는 주는 발표 시점보다 2주 이상 전이다.
  // D+1에 돌 때는 "이번 주"가 대충 맞는 말이었지만 이제는 명백히 틀린 말이 된다.
  // 기간은 개시부의 periodLabel("8월 24~30일")이 절대 날짜로 이미 말하고 있다.
  // "직전 주"(비교 대상)는 여기 걸리지 않는다 — 그건 코드가 만들어 넘기는 어구다.
  [/이번\s*주|금주|지난\s*주|저번\s*주|요즘|최근\s*한\s*주/, '상대 시점 표현'],
