...
export function normalizeComplexName(name: string): string {
  return name
    ...
    .replace(/\s+(\d+단지)$/, '$1')      // 단지 앞 공백만 제거, 번호는 유지
    .replace(/\s*\d+차$/, '')            // "1차" "2차" 접미어 제거
    // 창원/김해 하위 동·읍면 접두어(진영/장유/율하/삼계/내서)는 기존 유지.
    // Phase 33 신규 16개 지역 시/군명 접두어 추가 — 동·읍면 단위 별칭은 지역별 리서치 필요해 defer
    .replace(/^(창원|김해|마산|진해|진영|장유|율하|삼계|내서|진주|통영|사천|밀양|거제|양산|의령|함안|창녕|고성|남해|하동|산청|함양|거창|합천)\s*/u, '')
    .replace(/\s+/g, '')
