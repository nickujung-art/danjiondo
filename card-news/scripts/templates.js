...
// 브라우저 iframe 미리보기용 — CDN 폰트 (file:// 경로 사용 불가, Pitfall-2 해결)
...
// 미리보기용 HTML 래퍼 — BASE_CSS_PREVIEW(CDN 폰트) 사용
function htmlPreview(body, css = '') {
  return `<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="UTF-8">
...
function BrandLockupPreview({ size = 46, fontSize = 30, color = 'var(--ink)' } = {}) {
  return `<div style="display:flex;align-items:center;gap:12px;">
...
// ── 미리보기 전용 렌더러 (CDN 폰트, iframe srcDoc용) ──────

/**
 * 1. 커버 카드 — 미리보기 (iframe srcDoc, CDN 폰트)
 */
...
/**
 * 2. TOP 3 하이라이트 — 미리보기 (iframe srcDoc, CDN 폰트)
 */
export function renderHighlightPreview(data) {
  const { week, region, area, period, source, ranking } = data
  const r = pad10(ranking ?? [])
  const top3 = r.slice(0, 3)
...
/**
 * 3. 전체 순위 1~10위 — 미리보기 (iframe srcDoc, CDN 폰트)
 */
export function renderRankingPreview(data) {
  const { region, area, period, source, ranking, seriesType } = data
  const r = pad10(ranking ?? [])
...
/**
 * 4. 클로징 CTA — 미리보기 (iframe srcDoc, CDN 폰트)
 */
